  ...
  draw: function(id, events, options){
    ...
    .attr("cx", function(d){
      ...
      if(cfg.dateDimension){
        ...
        var dateValue = (datetime != "")?("<h3>"+d.name +"</h3><small>("+datetime+")</small><p class='tsk_description'>"+d.description+"</p>"):d.name;
      }else{
    ...
    if(cfg.showLabels != false){
      ...
      svg.append("text")
         .text(startString).style("fill","var(--primary-text-color)")
         .attr("x", function(d){if(cfg.horizontalLayout){return d3.max([0, (margin-this.getBBox().width/2)])} return 0 /*Math.floor(this.getBBox().width/2)*/})
      ...
      svg.append("text")
         .text(endString).style("fill","var(--primary-text-color)")
         .attr("x", function(d){if(cfg.horizontalLayout){return  cfg.width -  d3.max([this.getBBox().width, (margin+this.getBBox().width/2)])} return 0/*Math.floor(this.getBBox().width/2)*/})
